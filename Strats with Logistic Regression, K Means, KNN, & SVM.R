...
library(e1071)
model.svm<-svm(norm_isdji,as.factor(isdir))
model.svm

#setups of normalized data and directions are same as KNN above
pred<-predict(model.svm,norm_osdji)
head(pred)

model.svm$SV #output vectors generated by SVM
model.svm$index #corresponding index values
model.svm$coefs
